...
type SaasAdminUserFormProps = {
  isOpen: boolean
  selectedAdmin: SaasAdminListItem | null
  isSubmitting: boolean
  onSubmit: (payload: CreateSaasAdminRequest | UpdateSaasAdminRequest) => Promise<void>
  onClose: () => void
}
...
export function SaasAdminUserForm({
  isOpen,
  selectedAdmin,
  isSubmitting,
  onSubmit,
  onClose,
}: SaasAdminUserFormProps) {
  ...
  async function handleSubmit(event: React.FormEvent<HTMLFormElement>) {
    ...
    if (isEditing) {
      const payload: UpdateSaasAdminRequest = {
    ...
    >
      ...
      >
        <div className="flex items-start justify-between gap-4 border-b border-slate-100 px-6 py-6 dark:border-slate-800 sm:px-8">
          <div>
            <h3 className="text-2xl font-bold tracking-tight text-slate-900 dark:text-slate-100">
              {isEditing ? 'Editar Usuário SaaS' : 'Novo Usuário SaaS'}
            </h3>
            <p className="mt-1 text-sm font-medium text-slate-500 dark:text-slate-400">
              Gestão de acessos com perfil SAAS_ADMIN
            </p>
          </div>
          ...
          <div className="grid grid-cols-1 gap-8 md:grid-cols-2 md:gap-10">
            ...
            <div className="space-y-6">
              ...
              <div className="rounded-2xl border border-slate-200 bg-slate-50 px-4 py-4 dark:border-slate-700 dark:bg-slate-800/40">
                <Switch
                  label={form.status ? 'Usuário SaaS ativo' : 'Usuário SaaS inativo'}
                  description={
                    form.status
                      ? 'Usuário liberado para acessar o painel SaaS.'
                      : 'Usuário bloqueado para acesso ao painel SaaS.'
                  }
                  checked={form.status}
              ...
              <div className="inline-flex items-center gap-2 rounded-xl bg-indigo-50 px-3 py-2 text-[11px] font-semibold text-indigo-700 dark:bg-indigo-900/30 dark:text-indigo-300">
                <Shield size={14} />
                Perfil com acesso administrativo ao SaaS
              </div>
            </div>
          </div>
        </div>
        ...
        <div className="flex flex-col-reverse gap-2 border-t border-slate-100 px-6 py-5 dark:border-slate-800 sm:flex-row sm:justify-end sm:px-8">
          ...
          >
            {isEditing ? 'Salvar alterações' : 'Salvar usuário SaaS'}
          </Button>
        </div>
      </form>
    </div>
